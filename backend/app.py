...
def diagnosis(email):
    try:
        ...
        # user = user_profiles.find_one({"email": "eshaan.vimal@gmail.com"})
        # if not user or 'dna_file_id' not in user:
        #     return "DNA file not found for this user", 404
        
        # dna_file_id = user['dna_file_id']
        # dna_file = fs.get(dna_file_id) 
        
        # dna_file_io = BytesIO(dna_file.read())
        # dna_file_io.seek(0)
        # dna_file_io.getvalue().decode('utf-8', 'ignore')
        ...
        # pdf_binary = Binary(pdf_io.read())

        # Now you need to update the user document to store the PDF binary content
        # Check if 'report_history' exists in the user profile
        # if "report_history" not in user:
        #     user_profiles.update_one(
        #         {"email": email},
        #         {"$set": {"report_history": [{"pdf": pdf_binary, "date": datetime.utcnow()}]}}
        #     )
        # else:
        #     user_profiles.update_one(
        #         {"email": email},
        #         {"$push": {"report_history": {"pdf": pdf_binary, "date": datetime.utcnow()}}}
        #     )
